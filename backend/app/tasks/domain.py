    ...
    try:
        if "spiderfoot" in selected_tools:
            try:
                ...
                rows = [result(case_id, "spiderfoot", domain, e["type"], e["data"], "medium", {"module": e["module"]}) for e in asyncio.run(run_domain_scan(domain))]
        ...
        if "theharvester" in selected_tools:
            try:
                ...
                rows = [result(case_id, "theharvester", domain, "email_enumeration", None, "medium", {"associated_email": item}) for item in report.get("emails", [])] + [result(case_id, "theharvester", domain, "subdomain", item, "medium", {"subdomain": item}) for item in report.get("hosts", [])]
